...
st.markdown(f"""<p style="font-size: 1.1rem; color: {ENKA_DARK};"><span style="color: {ENKA_GREEN}; font-weight: bold;">ENKA Energy Transition</span> - Battery Energy Storage System Analytics Platform</p>""", unsafe_allow_html=True)

# Problem Statement
st.header("🎯 Problem Statement")
st.markdown("""ENKA operates a growing fleet of Battery Energy Storage Systems (BESS) across multiple sites with different controller vendors. Current challenges include:
...
with col1:
    with st.container(border=True):
        ...
        st.markdown("""- **TMEIC Controller** - PCS/Plant Control telemetry
...
with col2:
    with st.container(border=True):
        ...
        st.markdown("""- **Bronze Layer** - Raw JSONL micro-batches
